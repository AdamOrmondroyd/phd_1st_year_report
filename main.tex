...
Once I have finished studying the dark energy equation of state, it is only natural to probe the equations of state of the rest of the dark sector: dark matter and dark radiation, parameterised again by a linear spline. This will be less straightforward than for dark energy, as \texttt{CAMB} does not include options for varying the equation of state parameter for components other than dark energy. Therefore, I will need to modify both \texttt{CAMB} and \texttt{Cobaya} to add this feature. This work should form a suitable basis for my first paper.

If dark matter and dark energy indeed exist, it would seem possible that dark radiation would also exist to mediate interactions between such particles \cite{Ackerman_2009}. A transition in this radiation has the potential to provide answers to the Hubble tension, the disparity between the locally measured values of $H_0$, and those measured from CMB experiments \cite{Aloni_2022}. Theoretical work by W.E.V.~Barker, another member of the group, provides a gravitational mechanism for why a fluid with $w=1/3$ might exist in the early universe, and then decay \cite{Barker_2020}. A \textit{linf} can model both the periods before and after the transition freely, and provide insight into the nature of the transition itself. This transition may have occurred at very high redshift $\mathcal{O}(10^4)$, so a \textit{linf} in $w_\textrm{dark radiation}(\log{a})$ would be more appropriate. 
